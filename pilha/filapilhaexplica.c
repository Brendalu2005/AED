...
void juntarFilas(Pessoa **head1, Pessoa **head2, Pessoa **head3, Pessoa **tail3, Pessoa **headP1){
    ...
    while(temp != NULL){
        ...
        // usar &head3 transforma ele em um ponteiro de um ponteiro de um ponteiro
        temp = temp->next;
    }
...
//AUGUSTO VERSION: 
// void concatenar(pessoa **headFila1, pessoa **headFila2, pessoa **pilha1){
//     pessoa *temp = headFila1;
//     while(temp != NULL){
//         insert(pilha1, temp->nome);
//         temp = temp->next;
//     }
...
void intercalaFilas(Pessoa **head1, Pessoa **head2, Pessoa **headP2){
    ...
    while(temp1 != NULL || temp2 != NULL){ 
        //os nomes são inseridos de forma intercalada, já que os if são percorridos
        //intercaladamente e adicionam no mesmo head;
        if(temp1 != NULL){ //primeiro temp1
            addPilha(headP2, temp1->nome);  // adiciona direto na pilha representada por head1
            temp1 = temp1->next;
        }
        if(temp2 != NULL){// em seguida temp2
            addPilha(headP2, temp2->nome);  // adiciona direto na mesma pilha
            temp2 = temp2->next;
        }
    }
            // vai adicionando alternado até que ambas cheguem em Nulo
}
